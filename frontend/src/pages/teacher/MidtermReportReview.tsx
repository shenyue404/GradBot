...
const midtermDimensionLabelMap: Record<keyof MidtermEvaluation['dimensionScores'], string> = {
  progress: '进度完成度',
  quality: '成果质量',
  planning: '计划安排',
  problemSolving: '问题解决',
...
const MidtermReportReview: React.FC = () => {
  ...
  const fetchReports = async () => {
    try {
      setLoading(true)
      const response = await teacherAPI.getPendingMidtermReports()
  ...
  const handleAiEvaluation = async (report: MidtermReport) => {
    try {
      ...
      console.error('智能评分中期报告失败:', error)
      message.error('智能评分失败')
    } finally {
        ...
        {selectedReport && (
          <>
                      ...
                      {Object.entries(selectedReport.aiEvaluation.dimensionScores).map(([key, value]) => (
                        <div key={key} style={{ marginBottom: 8 }}>
                          <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                            <span>{midtermDimensionLabelMap[key as keyof MidtermEvaluation['dimensionScores']]}</span>
            ...
            <Form form={feedbackForm} layout="vertical">
              ...
              >
                <TextArea rows={5} placeholder="可先点击“智能生成评语”，再结合项目实际情况修改。" />
              </Form.Item>
