...
const proposalDimensionLabelMap: Record<keyof ProposalEvaluation['dimensionScores'], string> = {
  background: '研究背景',
  significance: '研究意义',
  researchStatus: '研究现状',
...
const proposalAnalysisLabelMap: Record<keyof ProposalEvaluation['detailedAnalysis'], string> = {
  background: '研究背景',
  significance: '研究意义',
  researchStatus: '研究现状',
  researchContent: '研究内容',
...
const ProposalReview: React.FC = () => {
  ...
  const fetchProposals = async () => {
    try {
      setLoading(true)
      const response = await teacherAPI.getPendingProposals()
  ...
  const handleAiEvaluation = async (proposal: Proposal) => {
    try {
      ...
      if (!response.success || !response.data) {
        message.error('智能评分失败')
        return
      }

      updateProposal(proposal.id, (item) => ({ ...item, aiEvaluation: response.data }))
      message.success('智能评分已完成')
    } catch (error) {
      console.error('智能评分开题报告失败:', error)
      message.error('智能评分失败')
    } finally {
  ...
  const handleGenerateOpinion = async () => {
    ...
    try {
      ...
      message.success('智能评语已生成，可继续修改')
    } catch (error) {
      console.error('智能生成评语失败:', error)
      message.error('智能评语生成失败')
    } finally {
        ...
        {selectedProposal && (
          <>
            ...
            {selectedProposal.aiEvaluation && (
              <>
                <Divider orientation="left">智能评分结果</Divider>
                      ...
                      {Object.entries(selectedProposal.aiEvaluation.dimensionScores).map(([key, value]) => (
                        <div key={key} style={{ marginBottom: 8 }}>
                          <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                            <span>{proposalDimensionLabelMap[key as keyof ProposalEvaluation['dimensionScores']]}</span>
                ...
                <Card size="small" title="详细分析" style={{ marginBottom: 16 }}>
                  <Collapse
                    items={Object.entries(selectedProposal.aiEvaluation.detailedAnalysis).map(([key, value]) => ({
                      key,
                      label: proposalAnalysisLabelMap[key as keyof ProposalEvaluation['detailedAnalysis']],
                      children: value || '暂无分析',
                    }))}
                  />
                </Card>
            ...
            <Form form={feedbackForm} layout="vertical">
              ...
              >
                <TextArea rows={5} placeholder="可先点击“智能生成评语”，再结合你的判断进行修改。" />
              </Form.Item>
